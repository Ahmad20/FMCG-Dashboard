...
get_product <- function(x){
  res <- GET(
    sprintf(
      "https://shopee.co.id/api/v4/search/search_items?by=pop&entry_point=ShopBySearch&limit=100&match_id=%s&newest=0&order=desc&page_type=shop&scenario=PAGE_OTHERS&version=2"
      ,x)
  )
  ...
  brand <- data$items$item_basic$brand
  normalize_prize <- function(x){
    x/100000
  }
  price <-sapply(price, FUN=normalize_prize)
  return (data.frame(item_id, name, sold, price, rating, brand))
}
